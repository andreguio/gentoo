...
wget https://distfiles.gentoo.org/releases/amd64/autobuilds/20240225T170409Z/stage3-amd64-openrc-20240225T170409Z.tar.xz -O /mnt/gentoo/stage3.tar.xz
...
#echo '' >> /etc/portage/make.conf
#echo 'U_PROFILE_IMP="-a52 -bluetooth -branding -cdda -cdr -cups -dvd -dvdr -ppds -qt5 -spell"' >> /etc/portage/make.conf
#echo 'U_VIDEO_PER="amdgpu mesa vaapi vdpau vulkan"' >> /etc/portage/make.conf
#echo 'U_VIDEO_IMP="-nvidia -nvidia-gpu -cuda -optix -nvenc -cg -cudnn -gfxnvidia"' >> /etc/portage/make.conf
#echo 'U_DESKTOP_PER="grub gtk2 gtk3 xfce thunar pulseaudio lm-sensors hddtemp"' >> /etc/portage/make.conf
#echo 'U_DESKTOP_IMP="-wayland -xwayland -gnome -kde -mate -cinnamon -motif -qt6 -plasma -nautilus -caja -gnome-keyring -networkmanager -handbook"' >> /etc/portage/make.conf
#echo 'U_OTHERS_PER="git libvirt qemu sudo"' >> /etc/portage/make.conf
#echo 'U_OTHERS_IMP="-accessibility -oss -jack -lilo -vivaldi"' >> /etc/portage/make.conf
#echo 'U_DEVICES_IMP="-gphoto2 -ipod -joystick -scanner -wifi -thinkpad -wireless -webcam"' >> /etc/portage/make.conf
#echo 'U_CORPS_IMP="-ios -aqua -coreaudio -mysql -oracle -samba -systemd -telemetry -pipewire"' >> /etc/portage/make.conf
#echo '' >> /etc/portage/make.conf
#echo 'USE="$U_PROFILE_IMP $U_VIDEO_PER $U_VIDEO_IMP $U_DESKTOP_PER $U_DESKTOP_IMP $U_OTHERS_PER $U_OTHERS_IMP $U_DEVICES_IMP $U_CORPS_IMP"' >> /etc/portage/make.conf
####
#### ==========================	11 - REPOSITÓRIO
####
